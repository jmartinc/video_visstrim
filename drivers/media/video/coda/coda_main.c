...
const struct v4l2_ioctl_ops *get_v4l2_ioctl_ops(void)
{
	return &coda_ioctl_ops;
}
...
int coda_isr(struct coda_dev *dev)
{
	struct coda_ctx *ctx;
...
struct v4l2_m2m_ops *get_m2m_ops(void)
{
	return &coda_m2m_ops;
}
...
int coda_ctrls_setup(struct coda_ctx *ctx)
{
...
static void coda_fw_callback(const struct firmware *fw, void *context)
{
	...
	vfd->fops	= &coda_fops,
	vfd->ioctl_ops	= get_v4l2_ioctl_ops();
	...
	dev->m2m_dev = v4l2_m2m_init(get_m2m_ops());
...
static int coda_remove(struct platform_device *pdev)
{
	...
	dma_free_coherent(&pdev->dev, bufsize, &dev->codebuf.vaddr,
			  dev->codebuf.paddr);
	clk_put(dev->clk);
